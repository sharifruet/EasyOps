@echo off
REM Start auth-service with correct Eureka configuration

cd /d "%~dp0\easyops-erp\services\auth-service"

set EUREKA_INSTANCE_HOSTNAME=host.docker.internal
set EUREKA_INSTANCE_PREFER_IP_ADDRESS=false
set SPRING_PROFILES_ACTIVE=local

echo Starting auth-service with Eureka hostname: host.docker.internal
